...
export default function PartnerJobListPage() {
  ...
  const handleSubmitJob = async (e) => {
    ...
    try {
      ...
      setShowModal(false); loadJobs(); alert("Berhasil diajukan!");
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl overflow-hidden max-h-[95vh] flex flex-col">
            ...
            <form onSubmit={handleSubmitJob} className="overflow-y-auto p-10 custom-scrollbar space-y-12">
               ...
               <div className="p-8 border-2 border-slate-50 rounded-[2.5rem] space-y-8">
                  ...
                  <div className="md:col-span-3"><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest block mb-1">Syarat Khusus (JLPT, SSW, dll)</label><textarea className="input-field bg-slate-50 border-none font-medium p-4 rounded-xl text-xs" rows="3" value={formData.syaratKhusus} onChange={(e) => setFormData({...formData, syaratKhusus: e.target.value})} /></div>
               </div>
