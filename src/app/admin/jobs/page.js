...
export default function JobManagementPage() {
  ...
  const dashboardStats = useMemo(() => {
    ...
    jobs.forEach(j => {
      // Sektor
      const b = j.bidang || "Lainnya";
      stats.sektor[b] = (stats.sektor[b] || 0) + 1;

      // Perusahaan
      const p = j.perusahaan || "N/A";
      stats.perusahaan[p] = (stats.perusahaan[p] || 0) + 1;

      // Nama Job
      const n = j.namaJob || "Job";
      stats.namaJob[n] = (stats.namaJob[n] || 0) + 1;
    ...
    <>
      ...
      <div className="max-w-full mx-auto px-6 py-8 bg-[#F8F9FC] min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
           {/* Nama Lowongan Top */}
           <div className="card p-6 border-2 border-white bg-white/60 backdrop-blur-sm rounded-[2.5rem] shadow-sm">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Top Lowongan</p>
              <div className="space-y-3">
                 {dashboardStats.namaJob.map(([n, count]) => (
                   <div key={n} onClick={() => {setFilterType("namaJob"); setFilterValue(n);}} className="flex justify-between items-center group cursor-pointer">
                      <span className="text-[11px] font-bold text-slate-600 group-hover:text-indigo-600 truncate mr-2">{n}</span>
                      <span className="bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-lg text-[10px] font-black">{count}</span>
                   </div>
                 ))}
              </div>
           </div>
           ...
           {/* Perusahaan Top */}
           <div className="card p-6 border-2 border-white bg-white/60 backdrop-blur-sm rounded-[2.5rem] shadow-sm">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Top Perusahaan</p>
              <div className="space-y-3">
                 {dashboardStats.perusahaan.map(([p, count]) => (
                   <div key={p} onClick={() => {setFilterType("perusahaan"); setFilterValue(p);}} className="flex justify-between items-center group cursor-pointer">
                      <span className="text-[11px] font-bold text-slate-600 group-hover:text-indigo-600 truncate mr-2">{p}</span>
                      <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-lg text-[10px] font-black">{count}</span>
                   </div>
                 ))}
              </div>
           </div>
              ...
              <div className="space-y-3">
                 {dashboardStats.sektor.map(([s, count]) => (
                   <div key={s} onClick={() => {setFilterType("bidang"); setFilterValue(s);}} className="flex justify-between items-center group cursor-pointer">
                      <span className="text-[11px] font-bold text-slate-600 group-hover:text-indigo-600 truncate mr-2">{s}</span>
                      <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded-lg text-[10px] font-black">{count}</span>
                   </div>
                 ))}
              </div>
           </div>
           ...
           >
              <div className={`p-4 rounded-3xl mb-4 ${filterType === 'domisiliIndo' ? 'bg-white/20' : 'bg-indigo-50 text-indigo-600'}`}>
                 <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064" /></svg>
              </div>
              <h3 className="text-4xl font-black">{dashboardStats.indo}</h3>
              <p className={`text-[10px] font-black uppercase tracking-widest mt-2 ${filterType === 'domisiliIndo' ? 'text-indigo-200' : 'text-slate-400'}`}>Domisili Indonesia</p>
           </div>
           ...
           >
              <div className={`p-4 rounded-3xl mb-4 ${filterType === 'domisiliJepang' ? 'bg-white/20' : 'bg-rose-50 text-rose-600'}`}>
                 <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
              </div>
              <h3 className="text-4xl font-black">{dashboardStats.jepang}</h3>
              <p className={`text-[10px] font-black uppercase tracking-widest mt-2 ${filterType === 'domisiliJepang' ? 'text-rose-200' : 'text-slate-400'}`}>Domisili Jepang</p>
           </div>
        </div>
      ...
      {/* MODALS REMAINS THE SAME (keeping existing functionality) */}
      {showModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-fadeIn">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl overflow-hidden max-h-[95vh] flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto p-12 custom-scrollbar space-y-12">
               {/* Form sections would go here - keeping it concise */}
               <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-6">
                     <label className="form-label text-[10px] font-black uppercase tracking-widest text-slate-400">Pilih Kategori Lowongan</label>
                  ...
                  <div className="space-y-6">
                     <label className="form-label text-[10px] font-black uppercase tracking-widest text-slate-400">Pilih Skema Prioritas</label>
               ...
               <div className="flex gap-4 justify-end pt-12 border-t border-slate-50">
                  <button type="button" onClick={closeModal} className="px-12 py-5 font-black text-slate-300 uppercase text-[10px] tracking-widest hover:text-slate-500">Batal</button>
                  <button type="submit" className="bg-slate-900 text-white px-20 py-5 rounded-[2rem] font-black shadow-2xl shadow-indigo-200 hover:bg-indigo-600 transition-all uppercase text-[10px] tracking-widest" disabled={submitting}>
