    ...
    <>
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 bg-[#F8F9FC] min-h-screen">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
          <div
            ...
            className={`cursor-pointer card p-6 relative overflow-hidden transition-all duration-300 ${
              !filterStatus ? "bg-gradient-to-br from-[#2D1B69] to-[#4C2A9E] text-white shadow-2xl scale-[1.02]" : "bg-white border-transparent hover:border-indigo-200"
            }`}
          >
            <div className="flex justify-between items-start">
              <div className={`p-2 rounded-lg ${!filterStatus ? "bg-white/20" : "bg-indigo-50"}`}>
                <svg className={`w-6 h-6 ${!filterStatus ? "text-white" : "text-indigo-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
              </div>
              <span className={`text-2xl sm:text-4xl font-black ${!filterStatus ? "text-white" : "text-gray-900"}`}>{stats.total}</span>
            </div>
            <h3 className={`mt-4 text-xs sm:text-sm font-black uppercase tracking-widest ${!filterStatus ? "text-white/80" : "text-gray-400"}`}>Total Kandidat</h3>
            <div className="mt-2 flex items-center text-[10px] sm:text-[11px] font-bold opacity-70">
            ...
            return (
              <div
                ...
                className={`cursor-pointer card p-6 border-2 transition-all duration-200 flex flex-col justify-between ${
                  isActive ? `ring-4 ring-offset-1 ${config.border} border-current shadow-2xl scale-[1.02]` : `bg-white ${config.border} hover:bg-gray-50 border-gray-100`
                }`}
              ...
              >
                ...
                <div>
                  ...
                  <div className="mt-2 text-[10px] sm:text-[11px] font-bold text-gray-400 flex justify-between">
        ...
        <div className="card p-8 mb-8 border border-indigo-50 shadow-xl bg-white rounded-[2rem]">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-400 font-bold uppercase tracking-widest mt-1">Visualisasi tahapan seleksi & status seluruh kandidat</p>
               </div>
            </div>
          ...
          {/* Pipeline Bar */}
          <div className="w-full h-10 bg-gray-100 rounded-2xl overflow-hidden flex mb-10 shadow-inner border-4 border-slate-50">
            {Object.entries(stats.byStatus).map(([status, count]) => {
              ...
              return (
                <div
                  key={status}
                  className={`${config.color} h-full border-r border-white/20 last:border-0 relative group cursor-help transition-all`}
                  style={{ width: `${percent}%` }}
                >
                  <div className="hidden group-hover:flex absolute -top-12 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[11px] font-black px-3 py-1.5 rounded-xl whitespace-nowrap z-10 shadow-2xl">
                    {status}: {count} ({Math.round(percent)}%)
                  </div>
                  {percent > 5 && (
                    <span className="flex items-center justify-center h-full text-xs sm:text-sm font-black text-white drop-shadow-md">
                      {Math.round(percent)}%
                    </span>
                  )}
                </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs sm:text-sm font-black text-gray-700 truncate uppercase tracking-tighter">{status}</span>
                  </div>
                  <div className="flex items-end justify-between mt-2">
                    <div className={`${config.bg} ${config.text} px-3 py-1 rounded-xl text-[11px] sm:text-xs font-black uppercase`}>
                      {count} <span className="opacity-60">Orang</span>
                    </div>
                    <span className="text-[11px] sm:text-xs font-black text-gray-400">{percent.toFixed(1)}%</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
           ...
           <div className="card p-6 border border-gray-100 shadow-xl bg-white rounded-[2rem]">
             <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3">
                   ...
                   <h3 className="text-lg sm:text-xl font-black text-gray-800 uppercase tracking-tight">Bidang Kerja</h3>
                </div>
                <span className="text-xs font-black bg-blue-50 text-blue-600 px-3 py-1 rounded-full uppercase">{uniqueBidang.length} Sektor</span>
             </div>
             <div className="space-y-5 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
               {Object.entries(stats.byBidang)
                 ...
                 <div
                   ...
                   className={`cursor-pointer group rounded-xl p-3 transition-all ${filterBidang === bidang ? "bg-blue-50 ring-2 ring-blue-100 shadow-sm" : "hover:bg-gray-50"}`}
                 >
                   <div className="flex justify-between text-xs sm:text-sm mb-2">
                     ...
                     <span className="text-gray-500 font-bold">{count} <span className="opacity-50">Kand</span> • {Math.round((count/stats.total)*100)}%</span>
                   </div>
           ...
           {/* Kategori Section */}
           <div className="lg:col-span-2 card p-6 border border-gray-100 shadow-xl bg-white rounded-[2rem]">
              <div className="flex justify-between items-center mb-8">
                 <div className="flex items-center gap-3">
                    <div className="p-2 bg-purple-50 rounded-lg text-purple-600">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                    </div>
                    <h3 className="text-lg sm:text-xl font-black text-gray-800 uppercase tracking-tight">Kategori Kandidat</h3>
                 </div>
              ...
              <div className="mt-8 pt-8 border-t border-gray-100">
                <button
                  ...
                  className="w-full py-5 bg-slate-900 text-white font-black rounded-2xl hover:bg-indigo-600 transition-all shadow-2xl shadow-indigo-100 uppercase text-xs sm:text-sm tracking-widest active:scale-95"
                >
        ...
        {/* Candidate List Table - Keeping font as is, user only asked for Dashboard stats */}
        <div className="card !p-0 overflow-hidden border border-gray-100 shadow-2xl bg-white rounded-[2rem] mb-20">
          <div className="p-6 border-b border-gray-100 bg-white sticky top-0 z-20">
             ...
             <div className="mt-4 flex justify-between items-center">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Menampilkan <span className="text-indigo-600 font-black">{filteredCandidates.length}</span> kandidat</p>
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-indigo-50 transition-colors group">
                      <td className="py-5 px-8">
                        <div className="flex items-center gap-4">
                           <div className="w-12 h-12 rounded-xl overflow-hidden shadow-md ring-2 ring-white shrink-0">
