    ...
    <>
      ...
      <div className="max-w-full mx-auto px-6 py-10 bg-[#F8F9FC] min-h-screen font-sans">
        <div className="flex justify-between items-center mb-10">
           ...
           <button onClick={loadCandidates} className="bg-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest border-2 border-white shadow-sm hover:border-slate-200 transition-all">Refresh Data</button>
        </div>

        {/* Status Cards Grid */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
           <div onClick={() => setFilterStatus("")} className={`cursor-pointer p-6 rounded-[2.5rem] border-4 transition-all ${!filterStatus ? 'bg-slate-900 border-slate-700 text-white shadow-2xl scale-105' : 'bg-white border-white shadow-sm'}`}>
        ...
        <div className="card p-10 bg-white border-2 border-white rounded-[3rem] shadow-xl shadow-slate-200/40 mb-10">
           <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] mb-8">Pipeline Progres Kandidat</h2>
           <div className="flex h-12 w-full rounded-2xl overflow-hidden border-4 border-slate-50">
              {Object.entries(stats.byStatus).map(([status, count]) => (
        ...
        <div className="card overflow-hidden !p-0 border-none shadow-2xl rounded-[3rem] bg-white">
           <div className="p-8 border-b border-slate-50 flex gap-4">
              <input className="flex-1 h-14 bg-slate-50 border-none rounded-2xl px-6 font-bold text-sm" placeholder="Cari nama kandidat..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} />
              {filterStatus && <button onClick={()=>setFilterStatus("")} className="bg-rose-50 text-rose-600 px-8 rounded-2xl font-black uppercase text-[10px] tracking-widest border border-rose-100">Reset Status ✕</button>}
           </div>
                    ...
                    {filteredCandidates.slice(0, 10).map(c => (
                      <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                         ...
                         <td className="py-5 px-8 text-center">
                            <Link href={`/admin/edit/${c.id}`} className="inline-block p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></Link>
                         </td>
                      </tr>
                    ))}
                 </tbody>
